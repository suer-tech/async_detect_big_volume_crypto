...
def get_crypto():
    ...
    for t in top:
        ...
        with open('crypto.txt', 'a') as file:
            if float(t['priceChangePercent']) > 0:
                file.write(f"{t['symbol']}: {t['lastPrice']}  +{float(t['priceChangePercent']):.2f}%\n")
            else:
                file.write(f"{t['symbol']}: {t['lastPrice']}  {float(t['priceChangePercent']):.2f}%\n")
    ...
    for gainer in top_gainers:
        # Открываем файл для записи
        with open('crypto.txt', 'a') as file:
            file.write(f"{gainer['symbol']}: {gainer['lastPrice']}  +{float(gainer['priceChangePercent']):.2f}%\n")
    ...
    for loser in top_losers:
        # Открываем файл для записи
        with open('crypto.txt', 'a') as file:
            file.write(f"{loser['symbol']}: {loser['lastPrice']}  {float(loser['priceChangePercent']):.2f}%\n")
